...
def predict_(sub_pre_date_array, train_models_name):
    ...
    for i in range(0,len(sub_pre_date_array)):
        ...
        res = pd.DataFrame(pre_y_array)
        nowTime = datetime.datetime.now().strftime('%m%d%H%M')  # 现在
        filename = 'Result/pre_y_array.csv'
...
def recovery_result(pre_y_array, affinity_Predict, sequence):
    ...
    recovery_pre_y = []
    pre_y_array_list = []
    for i in range(0,len(pre_y_array)):
        pre_y_array_list.append(pre_y_array[i].tolist())

    #恢复预测集顺序
    for i in range(0,len(sequence)):
        recovery_pre_y.append(pre_y_array_list[sequence[i]][0])
        pre_y_array_list[sequence[i]].remove(pre_y_array_list[sequence[i]][0])

    #整合输出成文件
    for i in range(0, len(affinity_Predict_list)):
...
if __name__=="__main__":
    ...
    #数据分层
    sub_train_data_array = trainset_split(train, [200,300,400,600,800,1000,1300,1500,1800])
    sub_pre_data_array, sequence = pre_split(pre_x,[200,300,400,600,800,1000,1300,1500,1800])

    #train_mech_name = train_model_for_every_sub_trainset(sub_train_data_array)

    train_mech_name = ['svr_0model','svr_1model','svr_2model','svr_3model','svr_4model','svr_5model','svr_6model','svr_7model','svr_8model']

    pre_y_array = predict_(sub_pre_data_array, train_mech_name)

    Df_affinity_Predict = pd.read_csv('data/df_affinity_test.csv')
